...
export function StreakHud({ totalLessons, doneCount }: { totalLessons: number; doneCount: number }) {
  ...
  return (
    <div className="rounded-xl border border-emerald-900/50 bg-slate-900/60 p-4 font-mono text-xs">
      <p className="text-emerald-400">
        DÍA {Math.min(dayNumber, 90)} / 90 · RACHA {state.streak} · CLASES {doneCount}/{totalLessons} ({pct}%)
      </p>
